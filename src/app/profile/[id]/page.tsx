...
import {BookMarkedIcon, CalendarIcon, ClockIcon, TextQuoteIcon, XCircleIcon} from "lucide-react";
...
const Page = () => {
    ...
    const he = users.find((m) => m._id === id ?? m.userId === id)
    ...
    useEffect(() => {
        if (!he || lessons.length === 0) return;

        const myLessons = lessons.filter((lesson) =>
            lesson.studentIds.includes(he._id) || lesson.mentorIds.includes(he._id) || lesson.studentIds.includes(he.userId) || lesson.mentorIds.includes(he.userId)
        );
    ...
    useEffect(() => {
        if (!he || allMarks.length === 0) return;

        const myMarks = allMarks.filter(comment =>
            comment.studentId === he._id || comment.mentorId === he._id || comment.studentId === he.userId || comment.mentorId === he.userId
        );
    ...
    const hiddenHomeworks = useQuery(api.lessons.getHiddenHomework, {studentId: he?._id || he?.userId}) ?? [];
    const hiddenLessonIds = React.useMemo(() => new Set(hiddenHomeworks), [hiddenHomeworks]);
    const {theme} = useTheme()
    return (
        <section className="relative z-10 pt-12 pb-32 flex-grow container mx-auto px-4">
            <Navbar/>
            <ProfileHeaderUser user={he}/>
                ...
                <div className="space-y-8">
                    ...
                    <div className="relative backdrop-blur-sm border border-border p-6">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-xl font-bold tracking-tight">
                                {" "}
                                {he?.role === 'mentor' ? <span className="text-primary">Проводимые </span> :
                    ...
                    <div className="relative backdrop-blur-sm border border-border rounded-lg p-6">
                        ...
                        {displayedLessons.map((item, index) => {
                            ...
                            return (
                                <Card key={index} className="hover:shadow-md transition-all p-2 mb-8 ">
                                    {/* CANDIDATE INFO */}
                                            ...
                                            {item.mentorIds.map((mentorId) => {
                                                ...
                                                return (
                                                    <div key={mentorId} className="flex items-center gap-2">
                                                        ...
                                                        <p className={`text-sm ${he._id === mentorId && 'text-primary/90 font-semibold'}`}>{mentor.name}</p>
                                            ...
                                            {item.studentIds.map((studentId) => {
                                                ...
                                                return (
                                                    <div key={studentId} className="flex items-center gap-2">
                                                        ...
                                                        <p className={`text-sm ${he._id === studentId && 'text-primary/90 font-semibold'}`}>{name}</p>
                    ...
                    <div className="relative backdrop-blur-sm border border-border p-6">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-xl font-bold tracking-tight">
                                {" "}
                                {he?.role === 'mentor' ? <span className="text-primary">Поставленные </span> :
                                    ...
                                    <Card className=" px-4 py-6 flex items-center gap-4">
                                        ...
                                        <div className="flex flex-col gap-12 w-full">
                                            ...
                                            {he?.role === 'mentor' ? (
                                                        ...
                                                        return (
                                                            <div key={studentId}
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <p className={`text-sm ${he._id === studentId && 'text-primary/90 font-semibold'}`}>
                                                                        {name}
                                                                    </p>
                                                                </div>
                                                                {studentMark?.rating && (
                                                ...
                                                (() => {
                                                    const studentMark = allMarks.find(
                                                        (mark) => mark.lessonId === item._id && mark.studentId === he._id
                                                    );
                                                    ...
                                                    if (!studentMark) return <div
                                                        ...
                                                        <h1 className={'text-2xl font-semibold'}>У ученика пока нету
                                                            оценок</h1>
                                                    </div>

                                                    const student = getStudentInfo(users, he._id);
                                                    const {image, initials, name} = getStudentDisplayInfo(student);
                    ...
                    {he?.role === 'student' && <>
                        ...
                        <div className="relative backdrop-blur-sm border border-border rounded-lg p-6">
                            <CornerElements/>
                            {displayedLessons.map((item, index) => {
                                // Находим учителя (по первому mentorId)
                                const mentorId = item.mentorIds[0]; // например, берем первого
                                ...
                                if (item?.homework && !hiddenLessonIds.has(item._id)) {
                                    return (
                                        <Card
                                            className=" p-4 mb-8 relative rounded-lg border border-slate-200 shadow-sm">
